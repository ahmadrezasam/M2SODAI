...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Swin-T with Cross-Validation')
...
def train_swin_cv():
    ...
    project_name = os.path.abspath(os.path.join(base_dir, 'runs_swin_hbb'))
    ...
    for fold in folds:
        print(f"\n{'='*50}")
        print(f"STARTING TRAINING FOR FOLD {fold} (Swin-T)")
        print(f"{'='*50}\n")
        
        from datetime import datetime
        current_date = datetime.now().strftime('%Y%m%d')
        config_path = f"configs/faster_rcnn/fold_{fold}_swin_t_3x_hbb.py"
        work_dir = os.path.join(project_name, f"swin_t_hbb_fold_{fold}_{current_date}")
