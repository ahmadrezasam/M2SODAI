...
from mmdet.models.detectors.zero_supervised_joint_detector import ZeroSupervisedJointDetector
from mmcv.utils import Config, ConfigDict

def test_zero_forward():
    print("Testing ZeroSupervisedJointDetector forward pass...")
    
    # Mock configuration
    norm_cfg = dict(type='GN', num_groups=32, requires_grad=True)
    model_cfg = dict(
        type='ZeroSupervisedJointDetector',
    ...
    print("Running forward_train (Zero-Supervised mode)...")
    ...
    assert len(losses) == 1, f"Expected only 1 loss (consistency), but got {len(losses)}: {list(losses.keys())}"
    
    print("\nTest passed successfully! No supervised losses were calculated.")
