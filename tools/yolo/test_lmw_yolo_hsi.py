...
def convert_coco_to_yolo_hsi(json_path, images_dir, labels_dir):
    ...
    for img_id, file_name in image_id_to_name.items():
        # Only process if image exists in HSI images dir
        ...
        with open(label_path, 'w') as f:
            if img_id in img_annotations:
                for ann in img_annotations[img_id]:
                    # COCO: [x, y, w, h] (top-left)
                    # YOLO: [class, x_center, y_center, w, h] (normalized 0-1)
                    # Image size is fixed 1600x1600 in our HSI projection
                    x, y, w, h = ann['bbox']
...
def main():
    ...
    yolo_hsi_dir = os.path.join(project_root, "data/yolo_hsi/test")
    hsi_images_dir = os.path.join(yolo_hsi_dir, "images")
    hsi_labels_dir = os.path.join(yolo_hsi_dir, "labels")
    gt_json = os.path.join(project_root, "data/test_coco/annotations.json")
    
    # 1. Convert annotations
    convert_coco_to_yolo_hsi(gt_json, hsi_images_dir, hsi_labels_dir)
    
    # 2. Create dataset.yaml
    dataset_yaml_path = os.path.join(yolo_hsi_dir, "dataset.yaml")
    with open(dataset_yaml_path, 'w') as f:
        f.write(f"path: {yolo_hsi_dir}\n")
    ...
    # 5. Run Evaluation
    print("Running HSI evaluation (Lower Bound)...")
