...
def _amp_checkpoint(fn, *args):
    """Gradient checkpoint that preserves the autocast (fp16) context.

    torch.utils.checkpoint loses the amp autocast context during the
    recomputation in the backward pass, causing dtype mismatches between
    fp16 activations and fp32 weights.  This wrapper captures the current
    autocast state and re-enables it inside the recomputed forward.
    """
...
class conv1_hsi(nn.Module):
    ...
    def forward(self, x):
        inp = x.unsqueeze(1)
        # Checkpoint each 3D conv branch to save memory from
        # intermediate 3D convolution activations
...
class TwoStageDetectorDFPN(BaseDetectorFinal):
    ...
    def _fuse_level(self, x_i, x_hsi_i, x_att_i):
        """Fuse RGB and HSI features at a single FPN level."""
        h, w = x_i.shape[-2:]
    ...
    def extract_feat(self, img, hsi):
        ...
        # Fuse per-level with checkpointing to reduce peak memory.
        # Gradient checkpointing recomputes activations during backward
        # instead of storing them, saving memory with no effect on accuracy.
        fused = []
        for i in range(len(x)):
            if x[i].requires_grad:
                fused_i = _amp_checkpoint(
                    self._fuse_level, x[i], x_hsi[i], x_att[i])
            else:
                fused_i = self._fuse_level(x[i], x_hsi[i], x_att[i])
            fused.append(fused_i)

        # Eagerly free intermediate feature maps
        del x, x_hsi, x_att
    ...
    def forward_train(self,
        ...
        # Release cached but unused GPU memory blocks to defragment VRAM.
        # Without this, variable-sized inputs cause fragmentation that
        # prevents large contiguous allocations in the RPN IoU computation.
