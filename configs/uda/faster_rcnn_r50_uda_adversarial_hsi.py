...
# We start from a pre-trained RGB model
load_from = 'work_dirs/faster_rcnn_r50_rgb/rgb_projected.pth'

norm_cfg = dict(type='GN', num_groups=32, requires_grad=True)

model = dict(
    type='UDAAdversarialDetector', # Using the new Adversarial detector
    backbone=dict(
    ...
    domain_discriminator=dict(
        ...
        hidden_channels=256,
        num_convs=3,
        grl_alpha=0.1 # Alpha for Gradient Reversal
    ),
    rpn_head=dict(
        type='RPNHead',
        in_channels=256,
    ...
    test_cfg=dict(
        ...
        rcnn=dict(score_thr=0.01, nms=dict(type='soft_nms', iou_threshold=0.5, min_score=0.01), max_per_img=100)
    )
)
